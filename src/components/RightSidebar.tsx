                  ...
                  <div className="text-xs space-y-1">
                    ...
                    {currentCoolingStats.airflowIssues.length > 0 && (
                      <div className="relative group">
                        ...
                        <div className="absolute left-0 bottom-full mb-1 hidden group-hover:block z-50 w-64 p-2 rounded bg-gray-900 border border-gray-600 shadow-lg text-xs text-gray-200 space-y-1">
                          {currentCoolingStats.airflowIssues.map((issue, i) => (
                            <div key={i} className="flex items-start gap-1">
                              <AlertTriangle size={10} className={`mt-0.5 flex-shrink-0 ${issue.severity === 'high' ? 'text-red-400' : issue.severity === 'medium' ? 'text-yellow-400' : 'text-blue-400'}`} />
                              <span>{issue.unit > 0 ? `U${issue.unit} ` : ''}{issue.item}: {issue.issue}</span>
                            </div>
                          ))}
                        </div>
                      </div>
